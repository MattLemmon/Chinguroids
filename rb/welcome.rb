...
class Beginning < Chingu::GameState
  ...
  def setup
    ...
    $music.play(true)
    after (10) {
      push_game_state(Chingu::GameStates::FadeTo.new(OpeningCredits.new, :speed => 5))
    }
  end
end
...
class OpeningCredits < Chingu::GameState
  trait :timer
  def setup
    self.input = { :esc => :exit, [:enter, :return] => OpeningCredits2, :p => Pause, :r => lambda{current_game_state.setup} }
